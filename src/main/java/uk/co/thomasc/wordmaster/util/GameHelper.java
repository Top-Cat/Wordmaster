...
public class GameHelper implements GooglePlayServicesClient.ConnectionCallbacks, GooglePlayServicesClient.OnConnectionFailedListener, OnSignOutCompleteListener {
	...
	int mConnectedClients = GameHelper.CLIENT_NONE;

	// What client are we currently connecting?
	int mClientCurrentlyConnecting = GameHelper.CLIENT_NONE;
	...
	String mSigningInMessage = "Hold on, we're spinning...";
	String mSigningOutMessage = "But... you were one game away from a free kitten";
	...
	public void setup(GameHelperListener listener) {
		setup(listener, GameHelper.CLIENT_GAMES);
	}
	...
	public void setup(GameHelperListener listener, int clientsToUse) {
		...
		if (0 != (clientsToUse & GameHelper.CLIENT_GAMES)) {
			scopesVector.add(Scopes.GAMES);
		}
		if (0 != (clientsToUse & GameHelper.CLIENT_PLUS)) {
			scopesVector.add(Scopes.PLUS_LOGIN);
		}
		if (0 != (clientsToUse & GameHelper.CLIENT_APPSTATE)) {
	...
	public void onStop() {
		...
		// leaks!)
		killConnections(GameHelper.CLIENT_ALL);
	...
	public void signOut() {
		...
		killConnections(GameHelper.CLIENT_ALL & ~GameHelper.CLIENT_GAMES);
	}
	...
	void startConnections() {
		mConnectedClients = GameHelper.CLIENT_NONE;
		mInvitationId = null;
		connectNextClient();
	}
	...
	void connectNextClient() {
		...
		if (mGamesClient != null && 0 != (pendingClients & GameHelper.CLIENT_GAMES)) {
			debugLog("Connecting GamesClient.");
			mClientCurrentlyConnecting = GameHelper.CLIENT_GAMES;
	...
	void killConnections(int whatClients) {
		if ((whatClients & GameHelper.CLIENT_GAMES) != 0 && mGamesClient != null && mGamesClient.isConnected()) {
			mConnectedClients &= ~GameHelper.CLIENT_GAMES;
			mGamesClient.disconnect();
		}
		if ((whatClients & GameHelper.CLIENT_PLUS) != 0 && mPlusClient != null && mPlusClient.isConnected()) {
			mConnectedClients &= ~GameHelper.CLIENT_PLUS;
			mPlusClient.disconnect();
		}
		if ((whatClients & GameHelper.CLIENT_APPSTATE) != 0 && mAppStateClient != null && mAppStateClient.isConnected()) {
			mConnectedClients &= ~GameHelper.CLIENT_APPSTATE;
	...
	public void onConnected(Bundle connectionHint) {
		...
		// later retrieval.
		if (mClientCurrentlyConnecting == GameHelper.CLIENT_GAMES && connectionHint != null) {
	...
	void resolveConnectionResult() {
		...
		if (mConnectionResult.hasResolution()) {
			...
			try {
				...
				mConnectionResult.startResolutionForResult(mActivity, GameHelper.RC_RESOLVE);
	...
	Dialog getErrorDialog(int errorCode) {
		...
		return new AlertDialog.Builder(getContext()).setMessage(mUnknownErrorMessage).setNeutralButton(android.R.string.ok, null).create();
	}
