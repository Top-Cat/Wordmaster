...
public class IabHelper {
		...
		mServiceConn = new ServiceConnection() {
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				try {
					...
					int response = mService.isBillingSupported(3, packageName, IabHelper.ITEM_TYPE_INAPP);
					...
					response = mService.isBillingSupported(3, packageName, IabHelper.ITEM_TYPE_SUBS);
					if (response == IabHelper.BILLING_RESPONSE_RESULT_OK) {
					...
					if (listener != null) {
						listener.onIabSetupFinished(new IabResult(IabHelper.IABHELPER_REMOTE_EXCEPTION,
		...
		if (intentServices != null && !intentServices.isEmpty()) {
			...
			if (listener != null) {
				listener.onIabSetupFinished(
					new IabResult(IabHelper.BILLING_RESPONSE_RESULT_BILLING_UNAVAILABLE,
	...
	public void launchPurchaseFlow(Activity act, String sku, int requestCode,
		...
		launchPurchaseFlow(act, sku, IabHelper.ITEM_TYPE_INAPP, requestCode, listener, extraData);
	}
	...
	public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK && responseCode == IabHelper.BILLING_RESPONSE_RESULT_OK) {
			...
			if (purchaseData == null || dataSignature == null) {
				...
				result = new IabResult(IabHelper.IABHELPER_UNKNOWN_ERROR, "IAB returned null purchaseData or dataSignature");
				if (mPurchaseListener != null) {
					mPurchaseListener.onIabPurchaseFinished(result, null);
				}
				return true;
			...
			try {
				...
				if (!Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
					...
					result = new IabResult(IabHelper.IABHELPER_VERIFICATION_FAILED, "Signature verification failed for sku " + sku);
					if (mPurchaseListener != null) {
				...
				e.printStackTrace();
				result = new IabResult(IabHelper.IABHELPER_BAD_RESPONSE, "Failed to parse purchase data.");
				if (mPurchaseListener != null) {
					mPurchaseListener.onIabPurchaseFinished(result, null);
				}
				return true;
			...
			logDebug("Result code was OK but in-app billing response was not OK: " + IabHelper.getResponseDesc(responseCode));
			if (mPurchaseListener != null) {
			...
			logDebug("Purchase canceled - Response: " + IabHelper.getResponseDesc(responseCode));
			...
			logError("Purchase failed. Result code: " + Integer.toString(resultCode)
				+ ". Response: " + IabHelper.getResponseDesc(responseCode));
	...
	public Inventory queryInventory(boolean querySkuDetails, List<String> moreItemSkus,
		...
		try {
			...
			int r = queryPurchases(inv, IabHelper.ITEM_TYPE_INAPP);
			if (r != IabHelper.BILLING_RESPONSE_RESULT_OK) {
				throw new IabException(r, "Error refreshing inventory (querying owned items).");
			}

			if (querySkuDetails) {
				r = querySkuDetails(IabHelper.ITEM_TYPE_INAPP, inv, moreItemSkus);
				if (r != IabHelper.BILLING_RESPONSE_RESULT_OK) {
			...
			if (mSubscriptionsSupported) {
				r = queryPurchases(inv, IabHelper.ITEM_TYPE_SUBS);
				if (r != IabHelper.BILLING_RESPONSE_RESULT_OK) {
					throw new IabException(r, "Error refreshing inventory (querying owned subscriptions).");
				}

				if (querySkuDetails) {
					r = querySkuDetails(IabHelper.ITEM_TYPE_SUBS, inv, moreItemSkus);
					if (r != IabHelper.BILLING_RESPONSE_RESULT_OK) {
			...
			throw new IabException(IabHelper.IABHELPER_BAD_RESPONSE, "Error parsing JSON response while refreshing inventory.", e);
		}
	}
	...
	void consume(Purchase itemInfo) throws IabException {
		...
		if (!itemInfo.mItemType.equals(IabHelper.ITEM_TYPE_INAPP)) {
		...
		try {
			...
			if (token == null || token.equals("")) {
				...
				throw new IabException(IabHelper.IABHELPER_MISSING_TOKEN, "PurchaseInfo is missing token for sku: "
					+ sku + " " + itemInfo);
			}
			...
			if (response == IabHelper.BILLING_RESPONSE_RESULT_OK) {
				...
				logDebug("Error consuming consuming sku " + sku + ". " + IabHelper.getResponseDesc(response));
				throw new IabException(response, "Error consuming sku " + sku);
			}
		} catch (RemoteException e) {
			throw new IabException(IabHelper.IABHELPER_REMOTE_EXCEPTION, "Remote exception while consuming. PurchaseInfo: " + itemInfo, e);
		}
	}
	...
	int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus)
		...
		if (!skuDetails.containsKey(IabHelper.RESPONSE_GET_SKU_DETAILS_LIST)) {
			...
			if (response != IabHelper.BILLING_RESPONSE_RESULT_OK) {
				logDebug("getSkuDetails() failed: " + IabHelper.getResponseDesc(response));
				return response;
			}
			else {
